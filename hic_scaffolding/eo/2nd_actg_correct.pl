...
while (<IN3>){
	...
	if ( $assign ne "uncluster" && exists $hash2{$id}) {
		...
		if ($sm1>$sm2){
			print OUT $id."\t".$assign."\t"."uncluster\t"."doubt\n";
		}
		else {print OUT $id."\t".$assign."\t".$assign."\t"."pass\n";}
	}
		...
		if ($sn > $sp && $sn > $sq){print OUT $id."\t".$assign."\t"."cluster2\t"."pass\n";}
		elsif ($sp > $sn && $sp > $sq){print OUT $id."\t".$assign."\t"."cluster1\t"."pass\n";}
		else {print OUT $id."\t".$assign."\t"."uncluster\t"."uncluster\n";}
	}
	else {print OUT $id."\t".$assign."\t"."uncluster\t"."noActg\n";}
}
